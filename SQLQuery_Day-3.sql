...
VALUES
       (1, 'TAYYEB', 'TAHIR', 22 ,Null ),
       (2, 'UMER', 'AMIR' , 23 , Null),
	   (3, 'AKMAL', 'SAMEER', 24 , Null),
       (4, 'SAMEER', 'TALHA', 24 , Null),
	   (5, 'TALHA' , 'TAHIR', 32, Null),
	   (6, 'AQSA', 'TAZEEM' , 22, Null),
       (7, 'AISHA' , 'UMER', 42, Null),
	   (8, 'SHAHZAL' , 'TAZEEM', 22,Null),
       (9, 'TAZEEM', 'TAZEEM', 21, Null),
	   (10, 'SADAAN' , 'TAHIR', 32, Null);
...
ORDER BY Students.Course_ID DESC;
...
SELECT TOP 1 Courses.Course_Name FROM Courses
...
SELECT Students.student_ID,Students.First_NAME, Students.Last_NAME,Students.Age FROM Students
WHERE Students.Course_ID=(SELECT Students.Course_ID FROM Students Where Students.First_NAME='AQSA'
)
AND Students.First_NAME <> 'AQSA';
